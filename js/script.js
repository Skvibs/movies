...
newUrl = (elem) => {
    ...
    getData(newUrl).then(function (result) {
        ...
        for (let i = 0; i < 10; i++) {
            if (result.Search[i].Poster == 'N/A' || !result.Search[i].Poster) result.Search[i].Poster = '//skvibs.github.io/movies/img/no-poster.png';
            list += `
                    <div class="item">
...
$('form').submit(function () {
    ...
    getData(url).then(function (result) {
        if (result.Response != 'False') {
            ...
            for (let i = 0; i < 10; i++) {
                if (result.Search[i].Poster == 'N/A') result.Search[i].Poster = '//skvibs.github.io/movies/img/no-poster.png';
                list += `
                    <div class="item">
                        <img class="item__img" src="${result.Search[i].Poster}" alt="Movie image">
                        <h3 class="item__title">${result.Search[i].Title}</h3>
